...
<?
	...
	if (0 && $USER->IsAdmin()) {
		...
		$fileName = $_SERVER['DOCUMENT_ROOT'].'/test/russlovo_kemerovo.csv';
		if (file_exists($fileName)) {
			...
			while (!feof($fp)) {
				...
				$fp_number = test_fp_number($arStr[3]);			//***** Номер ФП
				$code_1c = quotes_clear($arStr[0]);
				$price = floatval($arStr[8]);

				if ($fp_number !== false) {
					$arBook[$fp_number] = array(
						'FP_NUMBER' => $fp_number,
						'CODE_1C' => $code_1c,
						'PRICE' => $price
					);
		...
		// Загружаем из базы учебники
		$res = CIBlockElement::GetList(
			false,
			array('IBLOCK_ID' => 5, 'SECTION_ID' => 104),
			false, false,
		...
		while ($arFields = $res->GetNext()) {
			$j++;
			$key = $arFields['PROPERTY_FP_CODE_VALUE'];
			if ($arBook[$key]) {
				$i++;
